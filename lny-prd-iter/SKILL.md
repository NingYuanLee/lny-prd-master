...
name: lny-prd-iter
description: >-
  创建新版本文件夹、交付范围初始壳、变更台账与委派清单；不评审范围，不写规格正文。
...
| 按用户输入创建 `delivery_scope.md` 的候选范围与 `pending/reviewing` 初始状态 | 评审范围、解决未决决策或更新既有版本 `delivery_scope.md`（属 R） |
...
1. 以 `versions/` 为准核对当前最新；新版本号须更大，否则失败。既有版本的范围维护交 `/lny-prd-review`，本步不借机新建版本。
2. 解析输入为九类台账行。
3. 创建目录；按模板写 `iteration_notes.md` 开篇、`delivery_scope.md`、三类台账、`eval_signals.md`。`delivery_scope.md` 必须保留模板中的产品经理填写说明；`本期开发` 行来自用户明确选择，`本期下线` 行来自明确的废弃 Feature；评审未完成时写 `pending/reviewing`，禁止推断为 `approved`。
4. 根四规范：变更记录各 +1 行；更新「文档版本」「最后更新」；**不**改索引与 §5/§6/§7。
5. 输出委派清单与路径；列出新增/修改页的 `pages_prd目标路径` 供 ⑤/⑦。范围需要产品评审时建议 R `/lny-prd-review`，不自动批准。
