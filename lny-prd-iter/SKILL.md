...
本步为 **⑩ `/lny-prd-iter`**。先只读核验当前最新版本已由⑧形成终态结论；结论为“通过”且有交付范围时还须已有有效⑨ SP；随后确认不存在未清台账，再创建版本壳、台账、根规范「变更记录」一行与委派清单。不创建或修改任何 `delivery_scope.md`，不写规格正文。完成后“继续”回总控，从②③④自动推进至⑤→⑦/Q-S→⑥→⑦/Q-P；Q-P通过时才询问是否进入⑧。全流程见 `lny-prd-master/SKILL.md`。
...
| 输出委派清单（②③④→⑤→Q-S→⑥→Q-P） | 初始化 `v1.0.0`（属 ①）；写 `prototypes/index.html`（属 ⑥） |
...
8. 输出委派清单与根工作源路径；列出新增/修改页的 `pages_prd目标路径` 供⑤与Q-S。后续自动推进至Q-P通过，再询问是否进入⑧；⑧最终“通过”经产品确认后才创建本版本 `delivery_scope.md` 并发布批准快照，有交付范围则再执行⑨。
