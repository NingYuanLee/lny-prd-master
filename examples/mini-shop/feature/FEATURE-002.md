...
|---------|-------------------|----------|----------|
| AC-1 | 「查看」打开只读详情弹窗 | BR-1 | API-AD-002 |
| AC-2 | 「快捷编辑」可改库存状态与上架日期并保存 | BR-1, BR-2 | API-AD-002, API-AD-003 |
| AC-3 | 「新增」进入整页表单，保存后回列表 | BR-2 | API-AD-003 |
| AC-4 | 「选择关联」打开列表弹窗并确认一条 | BR-3 | API-AD-001 |
