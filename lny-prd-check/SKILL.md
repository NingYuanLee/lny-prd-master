...
name: lny-prd-check
description: >-
  只读检查 PRD：文档一致性、功能性验收、估点信号与交付门禁。不判断需求是否值得做，不修改仓库文件。
...
- **负责**：三大块全检（文档性 | 功能性 | 估点与交付门禁）；判定产品链可估、FE/BE/迭代信号是否齐及外部导出门禁是否满足（**不计 SP**）。
- **禁止**：改规格/台账/原型/流水；本步计算或写入标准工时点（属 ⑨）。
- **不做产品评审**：不判断需求是否值得做、范围是否合理或是否应该批准；这些属于 R `/lny-prd-review`。
...
4. **估点与交付门禁**：§3.1 虚引用 + FE 三维 + BE 四维 +（有台账时）迭代信号；若用户目标涉及外部研发台账，另按 §3.5 的 `delivery_scope.md`、模块来源与独立评审状态判断“可导出”。只验证已记录事实，不替产品给出批准结论。
