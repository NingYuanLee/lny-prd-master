...
1. 解析工作版本；目录须已存在。
2. **文档性**：Read 产物路径契约与共享页型不变量；依次运行 `python <skillDir>/scripts/verify-artifact-paths.py <prdRoot>` 与 `python <skillDir>/scripts/validate-prd-project.py <prdRoot>`，再按 [`reference-checks.md`](reference-checks.md) §1.1→1.6。扫描命中逐项列为高优先级，不删除文件。语义扫描器负责索引/明细、Feature 引用及已有 SP 输入快照；人工检查继续覆盖其余语义。
