...
### 3.5 外部导出门禁（仅用户目标涉及外部研发台账时）

本节只验证已记录事实，不判断范围是否合理，也不把检查通过解释为产品批准。

| 检查项 | 规则 | 不通过建议 | 优先级 |
|--------|------|------------|--------|
| 交付范围文件 | 目标版本须存在 `delivery_scope.md`；缺文件不阻塞普通 PRD 验收，但外部导出结论必须写“未具备外部导出条件” | `/lny-prd-iter` 创建新版本与初始范围壳 | 高 |
| 范围批准门禁 | `delivery_scope.md` 须明确本期 Feature；范围及 Feature 行均 `approved`，确认/阻塞数为 0，未决决策全 `closed`。未批准或仍有 open 决策是产品门禁，不是文档格式错误 | `/lny-prd-review` 给出明确结论，产品确认后记录 | 高 |
| 生命周期匹配 | 当前最高版本中，`本期开发` 须对应 `active`，`本期下线` 须对应 `deprecated`；历史版本保留当时范围，不用当前生命周期反判 | 当前 Feature 事实错误交 `/lny-prd-feature`；范围选择错误交 `/lny-prd-review` | 高 |

门禁满足后才建议 `/lny-prd-yunxiao`；不得用 PRD 基础验收通过替代导出门禁。
