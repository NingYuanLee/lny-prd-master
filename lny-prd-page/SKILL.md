...
夹具 `examples/mini-shop/` 的 ASCII / 分区措辞模板。线框 **只定分区顺序**；⑥ 视觉以 `gold/` 为准（对标下限，禁止照搬演示功能，也禁止忽略金样）。体验细则见 `lny-prd-ui` **§1.3.3 / §1.4.3**。
...
| 列表 | PAGE-MP-002 | 搜索行+筛选行贴顶 → 页签按钮组 → 1:1 横卡 | 搜索/筛选/页签不进滚动区；半屏方向 |
| 详情 | PAGE-MP-003 | 返回叠 16:9 → 图文介绍 → 评论 → 贴底次要操作 | 时间行；附图 1:1、一排最多五张；需要看大图才写点图放大阅览 |
| 表单 | PAGE-MP-004 | 返回顶栏 → 全套表单控件分区 → 贴底提交 | 右侧安全距；少选项中间弹窗、多选项/日期/地区底半屏；滑动条/上传须点名；进度条不在本页 |
...
| 功能入口 | PAGE-MP-010 | 返回顶栏 → 分组入口行（通栏和/或一行两个） | 左图标+名称；右箭头；短说明可无；一行两个点名 |
...
| 表单（套件） | PAGE-AD-009 | 面包屑 → 全套表单控件分区 → 保存/取消 | 无页内大标题；滑动条/上传须点名；日期用月历不是底半屏 |
| 详情 | PAGE-AD-008 | 面包屑 → 16:9 图集 → 图文介绍 → 评论 → 次要操作 | 无页内大标题；时间行；附图 1:1、一排最多五张；需要看大图才写点图放大阅览 |
...
| 功能入口 | PAGE-AD-010 | 面包屑 → 分组入口行（通栏和/或一行两个） | 无页内大标题；左图标+名称；右箭头；短说明可无 |
