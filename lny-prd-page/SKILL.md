...
| 向导 | PAGE-MP-005 | 返回顶栏 → 步骤条+进步条 → 当前步表单 → 贴底上一步/下一步 | 最后一步才提交 |
| 设置 | PAGE-MP-006 | 封面顶栏 → 分组一行一项 | 两倍标题栏高度；不要 16:9 |
...
| 向导 | PAGE-AD-003 | 面包屑 → 步骤条+进步条 → 当前步表单 | 最后一步才提交 |
| 工作台 | PAGE-AD-004 | 面包屑 → 指标卡 → 趋势 → 待办短表 | 禁止写成 D1-1 商品表 |
| 树+内容 | PAGE-AD-005 | 面包屑 → 左树右表 | 点树只换右区 |
| 设置 | PAGE-AD-006 | 面包屑 → 分组一行一项 → 保存 | 无页内大标题 |
