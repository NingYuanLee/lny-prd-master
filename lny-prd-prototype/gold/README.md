...
| 树 + 表单（不分页） | `desktop-split.html`（`md-d1--split` 左树右 **分类表单**；总控左增根、右展开/收起切换；节点维护与拖到上/中/下。**不是**分类钮。夹具 `PAGE-AD-005`） |
...
| 树 / 章节 / 定位 选型 | 见 [`reference-page-types.md`](../../lny-prd-master/reference-page-types.md) **「树 vs 章节列表选型」** | 分类树→`mobile-tree`；章节目录→`mobile-chapter-list`；左分组右横卡→`mobile-locator` |
