...
| 模块间距 | L3 包 `md-module`；块间 `--md-module-gap`（16px）。**先内边距、后外边距**：优先块自身 `padding`，两侧都够则不必再加块间 `gap` |
...
- [ ] 审美必做 5 条满足  
- [ ] 搜索/顶栏/安全距符合词典  
- [ ] 列表区/功能区与规格一致（列表：非分组无限流、`--cover` / `--tile` / `--row` / 单行独立有缝、轻阴影；功能：成组有限入口、通栏/一行两个/金刚与页内签**无圆角阴影卡片壳**）  
- [ ] 加载/空/失败/按下可感知  
