...
**时间轴** `md-timeline`：`__item` + `__rail`（`__node` + `__line`）+ `__body`。`is-done` 已完成、`is-active` 当前（外圈高亮）、`is-origin` 起点（空心圆）、`is-path` 途经小圆点、`is-future` 待完成。默认可点节点切高亮；**物流只读页** `md-timeline--static`：节点顺序 **当前→待完成→历史**（倒序）；`is-active` 实心+外圈、待完成全灰、历史段主色竖线。若带 `data-target` / `data-section` 则滚动对应锚点。
