...
**时间轴** `md-timeline`：`__item` + `__rail`（`__node` + `__line`）+ `__body`。`is-done` 已完成、`is-active` 当前（外圈高亮）、`is-origin` 起点（空心圆）、`is-path` 途经小圆点、`is-future` 待完成。默认可点节点切高亮；**物流只读页** `md-timeline--static`：节点顺序 **已下单→已出库→已揽收→运输中→配送中→已签收**；起点与当前高亮，待完成全灰。若带 `data-target` / `data-section` 则滚动对应锚点。
