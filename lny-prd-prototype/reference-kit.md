...
**时间轴** `md-timeline`：`__item` + `__rail`（`__node` + `__line`）+ `__body`。`is-done` 已完成、`is-active` 当前（外圈高亮）、`is-origin` 起点（空心圆）、`is-path` 途经小圆点、`is-future` 待完成。默认可点节点切高亮；**物流只读页** `md-timeline--static`：**展示倒序**「当前→待完成→历史」（最新在上、起点在下）；链路语义 **已下单→已出库→已揽收→运输中→配送中→已签收**；当前实心+外圈，待完成段竖线全灰，历史段主色竖线+小圆点，起点底部空心。若带 `data-target` / `data-section` 则滚动对应锚点。
