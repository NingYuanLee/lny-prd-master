...
3. ASCII = 分区顺序；金样 = 视觉密度。功能以本页规格为准。`ui/PAGE`「视觉细节」只给 ⑨ 估点，⑥ 视觉不降档。触屏 §1.3.3；桌面 D1-1 §1.4.3。**需求描述 ≠ 可见标题**（`lny-prd-ui` **§1.3.5**）：Markdown 标题、L 层、Module、US、Feature 与否定约束都不是 UI 文案，除非 §2.3 /「结构与控件」明确声明准确的用户可见标题。写 HTML 前对照金样 DOM——**金刚区/通栏/服务条不加 `md-section-head`**（`gold/mobile-grid.html` 金刚 module、`gold/mobile-menu.html`）；可由 Tab、字段名、设置行自解释的区域不重复加标题。禁止把 US/Feature/规格分区词（「刷题入口」「功能入口区」）当 `md-section-head__title`；列表/详情等多模块页仅在规格明确声明准确标题时使用该**产品文案**（如「精选」「评论」）。
