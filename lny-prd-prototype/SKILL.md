...
| PAGE-AD-012 字段详情 | `gold/desktop-detail.html` 字段签 | 同详情壳；分组 `md-desc`；桌面 `--cols-2`；跨列 `--span`。一条对象。不要沉浸式、不要当表单、不要套 lists 的 `md-group-list` |
...
4. **舒适默认（§2.3 漏写也要落地）**：按 `lny-prd-ui` **§1.7.0**。骨架、空态插图、失败可重试、一个主按钮、按下态、浮层过渡、D1-1 语义列宽+`md-d1--list`、评论时间行+附图约 40px、一排最多五张。禁止发明新跳转/新字段/新弹窗。**有字段时必须按层级排版**。**区块间距（先内边距、后外边距）**：优先靠各块自身 `padding`；相邻功能块贴死或仍显挤时再补块间 `gap`/`margin`——**两侧都已有足够内边距则不必再加外边距**，仅一侧有则视情况补。滚动区用 `md-module` + `--md-module-gap`；固定顶区遵循套件（如 `md-list-toolbar`）。写完后目视检查，禁止零间距堆叠。用户未给设计规范 ≠ 可以画线框。点图预览只默认给详情页与横卡多行，不是凡图都预览。
