...
4. **舒适默认（§2.3 漏写也要落地）**：按 `lny-prd-ui` **§1.7.0**。骨架、空态插图、失败可重试、一个主按钮、按下态、浮层过渡、D1-1 语义列宽+`md-d1--list`、评论时间行+附图约 40px、一排最多五张。禁止发明新跳转/新字段/新弹窗。**有字段时必须按层级排版**。**区块间距**：相邻功能块（搜索↔筛选/页签、顶栏↔工具条、筛选↔列表、表单分组之间）不得贴死——滚动区用 `md-module` + `--md-module-gap`；固定顶区/工具条内用父级 `gap: var(--md-space)` 或套件已有规则（如 `md-list-toolbar`）。写完后目视检查「上下是否缺缝」。用户未给设计规范 ≠ 可以画线框。点图预览只默认给详情页与横卡多行，不是凡图都预览。
