...
| PAGE-MP-009 物流时间轴 | `gold/mobile-timeline.html` | `md-timeline--static` 只读；展示倒序「当前→待完成→历史」；链路六节点；`is-active` 当前、`is-future` 待完成、`is-path` 历史、`is-origin` 起点；右 **横卡文本**（无左图，正文可 `__photos`） |
