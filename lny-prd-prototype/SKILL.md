...
| PAGE-MP-010 我的/服务 | `gold/mobile-menu.html` | `md-set-page` 浅灰底；顶区 **`md-profile--me`**；**`md-svc-strip`** 服务条（2/3/4 等分、项间短竖线、上区 **`__value` 统计或 `__icon` 图标**）；功能入口通栏/`md-set-pair`；右可为文字或 **方形配图 `--thumb`**；组间距漏底 |
