...
| H2  | 用户明确要求估点 / 故事点 / SP / `/lny-prd-sp`（**估点**优先于演示措辞）                                                                      | **⑨ sp**：B、C、D 未齐则先 **§3.1** 再 Read `lny-prd-sp/SKILL.md`；已齐则只 ⑨ |
...
| G2  | `versions/{latest}/` 存在且 `ui_changes.md` 中有新增/修改页面、对应 `PAGE-*.html` 在 `prototypes/` 下不存在，且 **B、C、D 已满足**，且 **无** `待②`/`待③`/`待④`/`待⑤`，且对应 `pages_prd` 已存在或台账标明 `ui直出` | **⑥ prototype**：Read `lny-prd-prototype/SKILL.md`；按台账补原型+镜像；**不**覆盖用户明确要求 G |
...
**「继续」**：用户回复「继续」时 **重新执行 §3**；若命中 **§3.1 批** 或 **§3.2**，本轮执行整批；否则命中 **一条** 非批步骤。**不**要求用户自选子指令。
