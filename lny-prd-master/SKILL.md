...
| G2  | `versions/{latest}/` 存在且 `ui_changes.md` 中有新增/修改页面、对应 `PAGE-*.html` 在 `prototypes/` 下不存在，且 **B、C、D 已满足**，且 **无** `待②`/`待③`/`待④`/`待⑤`，且对应 `pages_prd` 已存在或台账标明 `ui直出` | 本轮是演示/原型目标，或用户明确要求生成/更新原型 → **⑥**。仅「继续」而无上述目标 → **只建议** ⑥，**不落盘** |
...
**「继续」**：用户回复「继续」时 **重新执行 §3**。命中 **§3.1 批**（未清 ②③④ 或规格缺口）则本轮批跑；命中 **§3.2** 仅当当轮仍是演示/原型目标。规格已齐且无未清 I2 时命中 **J**：**只建议** ⑤⑥⑦⑨，**不**自动落盘。**例外**：上一轮是 **H2**「是否补规格再估」的询问时，本轮回执仍走 **H2**（肯定 → §3.1 再 ⑨；否定 → 只 ⑨ 写 `—`），**不**落到 B/C/D。**不**要求用户自选子指令。
...
- 同一轮对话：默认完成 **一个** 判定步骤；**例外**为 **§3.1 规格三件套批**、**§3.2 目标驱动补链（可 ②→⑥）**、**§3.3 估点后只刷总入口**。结束时报：**已执行步（可多项）**、**下一步建议**、**将 Read 的 SKILL 路径**。
