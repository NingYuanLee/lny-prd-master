...
### 3.1 领域粒度门禁（新增/拆分 Module 前必做）

**默认决策是复用现有 Module。** Module 应少量、稳定、粗粒度，一个 Module 可以承载多个版本和多个 Feature；不能因为新增页面、操作或单项需求就同步增加 Module。

新建候选 Module 时依次判断：

1. 候选能力是否能放入现有 Module 的「范围内」，且不破坏其职责与核心对象？能则直接复用。
2. 是否拥有稳定的核心业务对象、独立业务规则或生命周期，而非现有对象的一次操作？
3. 是否能写出清晰的范围内/外，并需要通过明确业务契约与其他 Module 协作？
4. 是否代表可长期承载一组 Feature 的能力边界，而非当前页面、菜单或一个 Feature 的临时分类？

第 1 项能复用，或第 2～4 项缺少实质证据时，**不得新建 Module**。初期只有一个 Feature 不自动判错，但必须能说明稳定边界与后续能力族；否则并回现有 Module。

以下默认不是 Module：

- 页面、菜单、终端或展示区域，如「首页」「管理后台」「列表页」
- 动作或 CRUD 分类，如「查询」「编辑」「审核操作」
- 流程阶段或状态，如「支付后」「待审核」
- 单个 Feature 的改写名称
- 缓存、队列、上传、鉴权等技术组件或框架能力
- 只为整理目录而产生、没有独立业务边界的名称

Module 只保留职责、核心对象、范围、对外能力与依赖。`BR-*`、流程分支、状态处理、`STORY/FEATURE/AC/PAGE/API/EXT-*` 编号及具体交互全部写入 Feature 明细。若一个 Module 的定义需要这些细节才能说清，先把细节下沉 Feature；只有出现两套可独立演进的核心对象、规则和对外契约时才拆 Module。
...
> 每个 `MODULE-*` 使用一个同结构三级章节。字段保持边界级短句或短列表，不展开规则、流程与验收。模块事实只在这里维护；不要另建汇总注册表或 `module/MODULE-*.md`。
