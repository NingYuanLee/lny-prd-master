...
本步为 **⑨ `/lny-prd-sp`**。只写 `versions/{v}/sp_report.md`。建议在 ⑦ 产品链可估之后。全流程见 `lny-prd-master/SKILL.md`。变更记录表仅 ① 首行 / ⑧ 追加。
...
- **负责**：按版本计算 FE_SP / BE_SP；同版本重跑直接覆盖报告。
- **禁止**：改规格/台账/`eval_signals`/DDL/人天；产品链不可估时仍写报告但停算（数值用 `—`）。
...
3. 产品链门禁；不可估则按模板写 `—` 并结束。
4. Read [`reference-weights.md`](reference-weights.md) 计算；明细按小计降序。报告末可选附录「压缩候选」（不进合计、不换算人天、不宣布 MVP）。
