...
①～⑦ 保持原职责；需求评审作为正式 **⑧**，位于 **⑦ check** 之后；⑨ 对已确认范围估点，⑩ 才开启下一迭代。
...
| ⑦ | `/lny-prd-check` | 文档一致性检查 + 功能性验收 + 估点与交付门禁（只读报告） |
| ⑧ | `/lny-prd-review` | 基于规格与 ⑦ 检查证据评审需求价值、方案和交付范围；给出拟定结论，产品确认或提出调整后记录决议 |
| ⑨ | `/lny-prd-sp` | 按⑧确认的交付范围汇总 FE/BE 标准工时点（`1 SP = 1 有效工程小时`，`sp_report.md`） |
| ⑩ | `/lny-prd-iter` | 核验前版评审终态与应有 SP 后，创建新版本壳、变更台账和委派清单；不创建交付范围 |

**②③④** 为规格三件套，按 **②→③→④ 同轮批处理**（中间不问「继续」）。用户目标是原型/演示时，总控在**当前版本**静默补 ②③④⑤⑥，**禁止静默 ⑩**（只有明确说新迭代才建新版本）。⑥ 按用户指定页面范围执行；未指定时一次完成当前全部缺失业务页，并对每页独立执行规格、金样与验收门禁。

**职责顺序**：`STORY → FEATURE → AC → PAGE/API/EXT` 是价值与验收主线，`FEATURE → MODULE` 是领域归属；Story↔Feature 只在 Feature 明细维护，Module 边界只在 `feature_spec.md` 维护。UI 是跨 Feature 的横向展示聚合，API 是产品级接口需求；④ 收束并反查 ②③，⑤ 同时读取 Feature 验收线与 UI 页面线。⑦ 先检查客观完整性与交付证据，⑧ 再做产品判断。评审员给出明确结论，产品只需确认或指出修改项；“继续”不会误触批准。
...
```text
① 立项 → ②③④ 规格三件套（同轮批处理） → ⑤ 单页 PRD → ⑥ 原型生成 → ⑦ 检查验收 → ⑧ 需求评审 → ⑨ 估点 → ⑩ 新迭代（仅用户明确要求）
```
...
6. `/lny-prd-check`（⑦）— 产品评审前的客观检查；须显式执行
7. `/lny-prd-review`（⑧）— 基于检查证据评审需求或本期范围；产品确认或提出调整
8. `/lny-prd-sp`（⑨）— ⑧确认交付范围后按本版本估点
