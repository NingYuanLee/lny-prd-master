...
AI Agent Skills 驱动的产品需求文档（PRD）工作流工具包。文档按总分总分层：根 `*_spec` 为总 → `ui/` `api/` `feature/` 为分（`ui/PAGE` **只写布局**）→ `pages_prd` + 原型 + 根 `scope.html` 为收束的总。对人目标驱动（要原型则静默补规格，不拒绝跳步）；对 Agent **禁止用 HTML 代替规格**、禁止静默开新版本。适用于 Cursor、Claude Code、Codex 等支持 Agent Skills 的 AI 编程工具。

本仓库是 **技能包**，不是 PRD 项目。请另开空目录立项；**禁止在本仓库根目录执行 `/lny-prd-master`**。粒度对照见 [`examples/mini-shop/`](examples/mini-shop/)。变更见 [`CHANGELOG.md`](CHANGELOG.md)。
...
**②③④** 为规格三件套，可并行推进（总控 **同轮批跑**，中间不问「继续」）。用户目标是原型/演示时，总控在**当前版本**静默补 ②③④⑤⑥，**禁止静默 ⑧**（只有明确说新迭代才建新版本）。**⑨** 建议在信号齐且产品链可估后执行。
...
- **阶段 1 采信号、阶段 2 算 SP**：规格采齐估点信号；**⑨ `/lny-prd-sp`** 按版本套公式出分（可附压缩候选，不换算人天）
...
- **已有项目**：按状态检测路由；用户要原型则静默补 ②→⑥。回复「继续」进入下一轮。⑧ 之后「继续」会先清 ②③④ 委派，再 ⑤
...
每轮只需输入「继续」，Agent 自动推进下一步；**②③④** 会在同一轮连续完成，无需三次确认。目标为演示时同一轮可做到 ⑥。要原型时对人静默补规格（禁止用 HTML 代替规格）；只有明确说新迭代才走 ⑧；⑦ 须明确要求检查。
...
完整模板、权重与填表示例见 [`lny-prd-sp/reference-weights.md`](lny-prd-sp/reference-weights.md)。对话回报只需 FE_SP / BE_SP / 合计三行。
