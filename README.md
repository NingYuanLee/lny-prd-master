...
直接跟 AI 要一张原型图，得到的往往只有能点的页面：布局怎么定的、要跟哪些数据说话、功能到底包含什么，都还散在对话里，下一轮就对不齐。本包要的效果不同——**先把这三件事自动补成 spec，再画原型**：页面怎么排布、可能需要哪些接口做数据交互、整件功能怎么描述。对人，你看到的仍是能点的图；对研发和下一次 AI，依据是已经写下来的规格，而不是那张网页。你要演示时，Agent 会在背后把规格补齐再出图，而不是只丢一张 HTML。
...
规格只写「产品要什么」，不写网址怎么拼、报文长什么样、数据库怎么建。给研发和 AI 写代码、给人对照验收，都以规格为准。每一步只做自己职责内的事，不许越权，也不许悄悄开新版本。
