...
LNY-PRD 做的是 **vibe-spec-coding** 里的 **vibe-spec**：帮助审视产品想法，给出可直接确认的评审结论，再把确认后的产品决议写成规格，配一份能点的静态原型。规格进了机器，人就不必再当需求和 AI 之间的传话筒。你不必看懂代码，但要能看懂规格，才能评审、对照、验收、改一版。交给研发，或接着做 **ai-coding**。**本包不写代码，产品负责人保留最终确认权。**
...
3. **一条需求能从头讲到尾。** 用户或利益相关方为什么需要 → 对应哪项功能 → 归属哪个领域 → 怎样算做成了 → 在哪些页面和接口留下验证证据。这条链连得上，人和 AI 才对得齐；断了就估不准，也做不准。

这套结构融合四种研发驱动：规格驱动负责承载和治理事实；Story 驱动说明价值来源，角色可扩展到运营、合规、迁移责任方和技术维护者；领域驱动用 Module 划定边界并约束跨模块协作；测试驱动用 Feature 下的 AC 组织 PAGE/API/EXT 验证证据。四者不是替代关系，而是同一追溯模型的不同维度。

规格只写「产品要什么」，不写网址怎么拼、报文长什么样、数据库怎么建。本包可给产品评审建议，但最终方向和批准结论由人决定；也不出可上线代码、设计稿、OpenAPI / DDL、人天排期。登录、支付、权限默认当作现成框架能力，不展开。
...
①～⑩ 编号保持不变；**⑦ check** 改为横向质量门禁：⑤后执行 Q-S，⑥作者自检后执行 Q-P；⑧仍是产品评审，⑨对已确认范围估点，⑩才开启下一迭代。
...
| ④ | `/lny-prd-feature` | 领域模块定义与功能规格拆分（`feature_spec.md` + `feature/FEATURE-*.md`） |
| ⑤ | `/lny-prd-page` | 当前单页 PRD 工作源（根 `pages_prd/{终端}/PAGE-*`；PC/AD 壳仅在根 `_shell`） |
| ⑥ | `/lny-prd-prototype` | 高保真可交互原型（全端静态 HTML + MUI 套件；按目标范围一次完成，逐页对照 `pages_prd`） |
| ⑦ | `/lny-prd-check` | 横向只读门禁：Q-S检查规格，Q-P检查原型，full依次检查两者 |
| ⑧ | `/lny-prd-review` | 按首版/非首版证据层评审，执行PAGE版本范围原子性门禁；最终确认后发布批准快照 |
| ⑨ | `/lny-prd-sp` | 按⑧确认的交付范围汇总 FE/BE 标准工时点（`1 SP = 1 有效工程小时`，`sp_report.md`） |
| ⑩ | `/lny-prd-iter` | 核验前版评审终态与应有 SP 后，创建新版本壳、变更台账和委派清单；不创建交付范围 |

**②③④** 为规格三件套，默认自动链为 **②③④→⑤前检→⑤→⑦/Q-S→⑥作者自检→⑦/Q-P**。只有缺产品事实、明确“只要规格”，或Q-P通过准备进入⑧时才停；**禁止静默⑩**。⑥按用户指定页面范围执行，未指定时一次完成当前全部缺失业务页，并逐页执行规格、金样与验收门禁。

**职责顺序**：`STORY → FEATURE → AC → PAGE/API/EXT` 是价值与验收主线，`FEATURE → MODULE` 是领域归属；Story↔Feature 只在 Feature 明细维护，Module 边界只在 `feature_spec.md` 维护。UI 是跨 Feature 的横向展示聚合，API 是产品级接口需求；④收束并反查②③，⑤汇合两条规格线。⑦分别把守规格与原型，⑧再做产品判断；评审调整会回②③④并自动重走两道门禁，直到形成最终结论。

## 二、核心原则

- **目标驱动、规格先行**：对人要原型则先交付原型，缺口由 Agent 静默补规格；禁止用 HTML 代替规格，禁止静默开新版本
- **四驱动合一**：规格承载事实，Story 给出价值来源，Module 划定领域边界，Feature 下的 AC 定义完成并引用 PAGE/API/EXT 证据
- **事实只写一次**：Story 只在 `main_spec`，Module 只在 `feature_spec`，Story↔Feature 与 Feature↔Module 只在 Feature 明细；反向关系由检查器派生
- **领域少而稳、细节进 Feature**：④ 新建 Module 前必须先尝试复用现有边界；页面、终端、CRUD、流程阶段、技术组件和单 Feature 改名不得充当领域，规则、流程、AC 与 PAGE/API/EXT 细节全部下沉 Feature
...
- **PAGE版本范围原子性**：同一PAGE的本版受影响Feature必须全部纳入或全部不纳入。首版检查根规格中该页全部待发布active Feature；非首版依据当前三类change反查，上一版已发布且本期未变的Feature只作存量基线
- **只读检查**：⑦ `/lny-prd-check` 保留编号，不改任何文件；Q-S/Q-P失败时由责任步骤自动返修
...
| 领域 | 说明 |
|------|------|
| **需求结构化** | 将模糊想法转化为 Story、Module、Feature、AC 与 PAGE/API/EXT 可追溯的规格体系 |
| **UI 规划** | 梳理页面清单、分包结构、组件层级，产出可落盘为 `ui/` 明细的 UI 设计规格 |
| **接口需求定义** | 按终端拆解 API 需求（含第三方），产出带请求/响应语义的接口明细文档 |
| **领域与功能拆分** | 在 `feature_spec` 定义领域边界，再把 Story 拆为归属明确的 Feature；每个 Feature 有目标、流程与 AC |
| **需求评审** | 从问题证据、目标价值、范围优先级、方案替代、验收效果与风险维度形成明确结论包，产品确认后记录版本范围与评审状态 |
| **单页 PRD** | 面向具体页面的回归式需求文档，强制写明依赖路径与数据来源 |
| **可交互原型** | 全端静态 HTML + MUI 套件，**默认高保真**；触屏：TabBar 贴底无顶栏、胶囊避让、1:1 图、下沉 Banner、金刚区、左/底/右半屏；AD 含弹窗/签/下拉/日期；按目标范围一次完成并逐页验收 |
...
| **不跳过产品确认** | ⑧ 负责下拟定结论，产品无需重做分析，但必须确认后才能写成正式批准事实 |
...
- 已按 5.1 将本仓库的 10 个核心 `lny-prd-*/` 技能作为整包安装
...
10 个核心技能（十步工作流）是一个原子、同版本的安装包，禁止只更新其中一部分。仅支持用户级安装：
...
- **已有项目**：按状态检测路由；回复“继续”会从最早未通过环节恢复，并自动推进至Q-P通过。明确“只要规格”则停在Q-S；Q-P通过后询问是否进入⑧

### 5.3 推荐工作流

```text
① 立项 → ②③④ 规格三件套 → ⑤ 单页 PRD → ⑦/Q-S → ⑥ 原型 → ⑦/Q-P → ⑧ 需求评审 → ⑨ 估点 → ⑩ 新迭代（仅用户明确要求）
```
...
│
├── main_spec.md                    # 产品规格说明书（概述、需求故事、终端、三大规格入口）
├── api_spec.md                     # 接口需求索引（§4 终端对齐 + API/EXT 清单）
├── ui_manifest.md                  # UI 设计清单（页面/分包/组件索引）
├── feature_spec.md                 # 领域模块边界 + Feature 轻量索引
│
├── api/                            # 接口需求明细（③ 负责）
│   ├── API-MP-001.md               #   小程序接口
...
│   └── FEATURE-002.md
│
├── pages_prd/                      # 当前单页 PRD 工作源（⑤ 负责）
│   ├── _shell/                     #   仅当前桌面业务壳，不发布到版本快照
│   │   └── AD-shell.md
│   ├── MP/                         #   按终端与 PAGE ID 平铺，不按路由建目录
│   │   └── PAGE-MP-001.md
│   └── AD/
│       └── PAGE-AD-001.md
│
...
└── versions/                       # 版本管理（① 立项 / ⑩ 迭代）
    ...
    │   ├── delivery_scope.md       #     ⑧确认后才创建的本期范围与评审决议
    │   ├── sp_report.md            #     版本标准工时点（⑨ 负责；夹具已给出）
    │   ├── pages_prd/              #     ⑧发布的批准业务页只读快照
    │   │   └── MP/pages/index/index/PAGE-MP-001.md  # 真实路由投影；无 _shell
    └── v1.1.0/                     #   次版（⑩ 创建）
        ├── iteration_notes.md
        ├── delivery_scope.md       #     ⑧确认后创建；评审前不存在
...
根 `pages_prd/` 是唯一当前单页PRD工作源，按终端/PAGE ID平铺；根 `prototypes/` 是唯一当前原型。只有⑧最终“通过”确认后，批准业务页才按 `delivery_scope.md` 映射与真实UI路由发布到 `versions/{v}/pages_prd/`；不发布 `_shell`。版本目录不复制原型树，历史原型通过项目Git commit/tag回看。
...
|----|------|----------|--------------|------|--------|-------------------|------|
| PAGE-MP-004 | 新增 | 新建 | 无 | 商品详情页 | ② | pages_prd/MP/PAGE-MP-004.md | 待② |
| PAGE-MP-001 | 修改 | 修改 | 无 | 首页新增推荐模块 | ② | pages_prd/MP/PAGE-MP-001.md | 待② |
...
4. `/lny-prd-page`（⑤）— `pages_prd/MP/PAGE-MP-004.md`、`pages_prd/MP/PAGE-MP-001.md`
5. `/lny-prd-check`（⑦/Q-S）— ⑤完成后自动检查规格
6. `/lny-prd-prototype`（⑥）— Q-S通过后自动生成并作者自检
7. `/lny-prd-check`（⑦/Q-P）— ⑥完成后自动检查原型；通过后询问是否进入⑧
8. `/lny-prd-review`（⑧）— 基于两道门禁证据评审；调整则回②③④循环
9. `/lny-prd-sp`（⑨）— ⑧确认交付范围并发布批准快照后按本版本估点
...
`hour-v1` 约定 **1 SP = 1 个有效工程小时**：规格完整、现有工程与公共组件可用、开发者熟悉项目时，用于实现、开发自测、常规联调和常规评审修正的净投入。等待外部排期、需求/设计返工、环境事故及不可预见故障不计入。FE_SP + BE_SP 是总人时，不能直接换成日历工期或发布日期。

| 项 | 约定 |
|----|------|
| 阶段 1 | 信号可采集 + check 可检查；**不算 SP** |
| 阶段 2 | **⑨ `/lny-prd-sp`** 按⑧确认范围套公式打分 → `versions/{v}/sp_report.md` |
| PRD 永不写 | DDL、真实表名、HTTP/path/JSON/响应 code |
| 校准层 | 无真实样本时 FE/BE 系数均为 `1.0（暂定）`；至少 3 个已完成版本后先按 hour-v1 重算历史基准点，再取 `实际小时 ÷ 基准点` 的中位数分别校准；禁止沿用旧权重 SP |

### 产品可评估链（阻塞「不可估」）

`STORY → FEATURE → AC → PAGE / API / EXT`，且 `FEATURE → MODULE`

缺任一关键环 → check **三、估点与交付门禁** 判不可估。FE/BE 信号缺 → 只标「角色估点信号不全」，**不**因此把产品打成不可估。

### FE 三维信号（落点）

> **FE 三维**（非 AD，一页一行）：① 页面实现 · ② 页面体验 · ③ 页面数据。流程角色是业务属性，不再作为工时权重。页面基线固定 `0.5`；功能数量 `0/0.5/1/1.5`；交互体验 `0/0.5/1/2`；视觉细节 `0/0.5/1/1.5`；API 数量 `0/0.5/1/1.5`；编排 `0/0.5/1`。

| 维 | 因子 | 手填/采集事实源 |
|:--:|------|------------------|
| ① 页面实现 | 页面基线 | 每个非 AD 业务 PAGE 固定 0.5；`_shell` 不计 |
| ① | 功能数量 | **单页** 关联 FEATURE 个数 → `少量`(0～2) / `一般`(3～5) / `较多`(6～8) / `极多`(≥9) |
| ② 页面体验 | 交互体验 | **单页** `ui/PAGE`：`简单` / `标准` / `较复杂` / `极复杂` |
| ② | 视觉细节 | **单页** `ui/PAGE`：`粗糙` / `标准` / `精致` / `极精致` |
| ③ 页面数据 | API数量 | **单页** `pages_prd` §5/§7 → `少量`(0) / `一般`(0.5) / `较多`(1.0) / `极多`(1.5) |
...
> **② 自有API** = 按 API 计行：固定 API 基线 + 数据操作形态 + 特殊通道；服务对象不再重复增加基线。
> **③ EXT对接**独立承担第三方实现工时，API 的第三方联动只作完整性信号，禁止双计。**④ FEATURE规则**只计主路径之外的代码分支，AC 行数只审计、不计分。
...
| ④ | FEATURE规则 | **独立**：`max(分支数-1, 0)×0.5`；AC 表行数只审计 |

> **页数**：FE 按 **非 AD 页**估；BE ① = 仅 AD（页侧规模，非「页数×0.5」）。

AD 页默认归 BE 是“后端/全栈人员承担管理后台”的团队口径；实际由 FE 承担时只移动归属，同一页不得重复计点。
...
- **计分范围**：以⑧确认的 `delivery_scope.md` 为上限；无台账变更行 → 已确认 `本期开发` Feature 全量，有台账 → 范围内的本版变更。禁止回退到全部 active Feature 猜范围。对象只取一个实现系数，再乘存量数据影响；禁止复用、复制扩展和修改系数互相连乘。
- **产出**：`versions/{v}/sp_report.md`（FE_SP / BE_SP / 合计）；范围未确认时停止并回⑧，产品链不可估则停算；**同版本重跑直接覆盖**原报告。落盘后同一轮刷 `prototypes/index.html` 版本清单（⑥ 只刷总入口；无原型则跳过）。
- **权重表**：以 [`lny-prd-sp/reference-weights.md`](lny-prd-sp/reference-weights.md) 为准（可后校准）。摘要：

| 侧 | 要点 |
|----|------|
| FE | **三维**（非 AD）：①页面实现（固定基线 0.5 + 功能数量 0～1.5）· ②页面体验（交互 0～2；视觉 0～1.5）· ③页面数据（API数量 0～1.5；编排 0～1） |
| BE | ①～④ 分对象：① AD页面规模 · ② 自有API（固定基线 0.5，第三方转 EXT）· ③ EXT对接（只计主动联调/调研，不计等待）· ④ FEATURE额外分支（AC 不计分） |

| 系数 | 实现：独立新建 1 / 模板复用 0.6 / 同代码跨端适配 0.3 / 复制扩展 0.4 / 修改 0.6 / 废弃 0.5；存量：无 1 / 迁移 1.3 / 备份回滚 1.2 |
...
├── lny-prd-review/SKILL.md + reference.md  # ⑧ 需求与交付范围评审
