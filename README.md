...
九步编号保持稳定；`R /lny-prd-review` 是按需显式触发的横向评审门禁，不插入编号步骤。
...
| R | `/lny-prd-review` | 评审需求价值、方案与交付范围，给出完整拟定结论；产品确认后记录既有 `delivery_scope.md` |
...
| ⑧ | `/lny-prd-iter` | 新迭代管理（建版本壳 + 候选范围初始壳 + 变更台账 + 委派清单） |
| ⑨ | `/lny-prd-sp` | 按指定版本汇总 FE/BE 标准工时点（`1 SP = 1 有效工程小时`，`sp_report.md`） |

**②③④** 为规格三件套，按 **②→③→④ 同轮批处理**（中间不问「继续」）。用户目标是原型/演示时，总控在**当前版本**静默补 ②③④⑤⑥，**禁止静默 ⑧**（只有明确说新迭代才建新版本）。⑥ **每轮最多 3 个业务页**；未画完时「继续」只续原型，不重做规格。

**兼容落地节奏**：当前不重排九步编号，也不取消 ②③④ 同轮批处理。逻辑上，`FEATURE → AC → 验证证据` 是纵向验收主线，UI 是跨 Feature 的横向展示聚合，API 是实现契约；④ 收束并反查 ②③，⑤ 同时读取 Feature 验收线与 UI 页面线。R 可评审单个需求或版本范围，评审员先下明确结论，产品只需确认或指出修改项；“继续”不会误触批准。旧项目可继续读取，新增的模块、评审和交付范围字段在项目被迭代或准备导出时渐进补齐；只有云效导出门禁要求其完整。
...
产品评审或调整既有版本范围时使用 `/lny-prd-review`：评审员先给出包含范围取舍和状态变化的完整拟定结论。产品可直接确认，也可补事实、调范围、改条件或质疑判断；评审员重评并生成完整修订结论，循环到确认后再更新该版本 `delivery_scope.md`。`/lny-prd-iter` 只创建新版本及其候选范围初始壳，云效适配器始终只读。
...
- **评审员下结论、人确认**：R 必须给出唯一拟定结论和落盘预览；产品回复确认后成为正式决议，未确认不得写 `approved`
...
| **不跳过产品确认** | R 负责下拟定结论，产品无需重做分析，但必须确认后才能写成正式批准事实 |
...
10 个核心技能（九步工作流 + R 横向评审）与 1 个可选云效适配器是一个原子、同版本的安装包，禁止只更新其中一部分。仅支持用户级安装：
...
6. `/lny-prd-review`（R）— 需求或本期范围需要产品评审时显式执行
7. `/lny-prd-check`（⑦）— 建议
8. `/lny-prd-sp`（⑨）— 本版本估点（可选）
...
- **计分范围**：无台账变更行 → **全量** active 规格；有台账 → **仅本版变更**。对象只取一个实现系数，再乘存量数据影响；禁止复用、复制扩展和修改系数互相连乘。
...
├── lny-prd-review/SKILL.md + reference.md  # R 需求与交付范围评审
