...
**②③④** 为规格三件套，可并行推进；**⑥ 默认依赖 ⑤**（`pages_prd`；例外标注 `ui直出`）。
...
- **阶段 1 可评估、不计点**：规格采齐估点信号；**不算故事点**（阶段 2 再公式打分）
...
| 阶段 2 | FE / BE 各自公式打分（未实施） |
| PRD 永不写 | DDL、真实表名、HTTP/path/JSON/响应 code |
| 校准层 | FE 细视觉看设计稿；BE 细持久化看实现/Superpower 出表后复估 |

### 产品可评估链（阻塞「不可估」）

`STORY → FEATURE → AC → PAGE → API`

缺任一关键环 → check **产品就绪度** 判不可估。FE/BE 信号缺 → 只标「角色估点信号不全」，**不**因此把产品打成不可估。
...
|:--:|------------------|
| ① 页数 | `ui_manifest` |
| ② 单页 API 数 | `pages_prd` §5 |
| ③ 请求编排 | `pages_prd` §7 |
| ④ 交互体验 | `ui/PAGE` + manifest（低/中/高） |
| ⑤ 视觉细节 | 同上（低/中/高；无阴影字号清单） |
| ⑥ 流程 | FEATURE **分支数** + pages_prd **流程角色** |

### BE 估点信号（落点）

| 维 | 信号 | 落盘 |
|:--:|------|------|
| B1 | 业务 PAGE 数 | FEATURE↔PAGE |
| B2 | 谁用 | API **服务对象**：`前端视图` / `资源管理` / `二者`（禁终端推导） |
| B2′ | 怎么做 | API **数据操作形态**六类：单对象查询 / 多对象关联查询 / 第三方联动查询 / 简单写入 / 多对象写入 / 第三方联动写入 |
| B3 | 第三方通道 | `EXT-*` + **对接复杂度** |
| B4 | 复杂 Feature | FEATURE **分支数** + AC/BR 密度 |
| B5 | 持久化粗档 | FEATURE **持久化复杂度** 低/中/高（无 DDL） |
...
产品就绪度 | FE 六维信号 | BE 信号 | （有迭代台账时）迭代估点信号
...
| **不做项目管理** | 没有甘特图、人员排期；迭代台账只是记录层面的轻量留痕；**阶段 1 亦不输出故事点分数**（只采估点信号） |
