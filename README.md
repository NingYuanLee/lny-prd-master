...
| 校准层 | FE 细视觉看设计稿；BE 表结构细活看实现/Superpower —— **均不进** FE 六维 / BE 四维枚举 |
...
> **FE 纯按页**（非 AD）；与 BE 维号独立。旧「页基线 + 流程角色」并为 **① 页面角色基线**。

| 维 | 名称 | 手填/采集事实源 |
|:--:|------|------------------|
| ① | 页面角色基线 | 非 AD 的 PAGE + `pages_prd` §7 流程角色 → `主流程`/`支线`/`辅助`/`设置` |
...
| 侧 | 要点 |
|----|------|
| FE | **纯按页**①～⑥：①页面角色基线（含流程角色）+ API数量/编排 + 交互/视觉 + 功能数量 |
| BE | ①～④ 分对象：① AD页面规模 · ② 自有API · ③ EXT对接 · ④ FEATURE规则 |
...
- **⑦ check**：**一文档性** | **二功能性** | **三产品就绪度**（含 FE 六维、BE 四维与迭代信号）—— 第三项 **不计点**
...
|----------|-------------:|---------:|----------:|----------:|----------:|----------:|---------:|-----:|
| PAGE-MP-001 | 2.5（主流程） | 1.5（一般） | 0.5（单请求或并行） | 2（标准） | 2（标准） | 0.5（少量） | 0.6 | 5.4 |
| PAGE-MP-004 | 2.0（支线） | 0.5（少量） | 0.5（单请求或并行） | 2（标准） | 3（精致） | 0.5（少量） | 1.0 | 8.5 |
| **小计（FE）** | | | | | | | | **13.9** |
...
|---------|--------:|-------------:|-----------:|---------:|---------:|-----:|
| API-MP-002 | 1（前端视图） | 1（多对象关联查询（普通）） | 0（否） | 0（否） | 0.6 | 1.2 |
| API-MP-010 | 1.5（前端视图和资源管理） | 2（多对象写入（复杂）） | 1（是） | 0（否） | 1.0 | 4.5 |
| **小计（②）** | | | | | | **5.7** |
