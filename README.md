...
| 校准层 | FE 细视觉看设计稿；BE 表结构细活看实现/Superpower —— **均不进**七维枚举 |
...
> **①** 非 AD 页计数；**②～⑦** 均 **按单页** 采集后汇总。

| 维 | 手填/采集事实源 |
|:--:|------------------|
| ① 页数（非AD） | `ui_manifest` 中终端 ≠ AD 的 PAGE |
| ② API 数量档位 | **单页** `pages_prd` §5 → `少量`/`一般`/`较多`/`极多`（0～1 / 2～3 / 4～6 / ≥7） |
| ③ API 请求编排类型 | **单页** `pages_prd` §7：`单请求或并行` / `串行` / `串并行混合` |
| ④ 页面交互体验档位 | **单页** `ui/PAGE` + manifest：`简单` / `标准` / `较复杂` / `极复杂` |
| ⑤ 页面视觉细节档位 | **单页** `ui/PAGE` + manifest：`粗糙` / `标准` / `精致` / `极精致`（无阴影字号清单） |
| ⑥ 功能数量档位 | **单页** 关联 FEATURE 个数 → `少量`/`一般`/`较多`/`极多`（同②分界） |
| ⑦ 流程角色 | **单页** `pages_prd` §7：`主流程` / `支线` / `辅助` / `设置` |

### BE 估点信号（落点）

维号与 FE 同用 **①～⑦**（报告分 FE/BE 两节）。**不采持久化档**。**①⑥⑦ 独立**；**②** 为 API 总数/清单；**③④⑤** 对 **② 中每个单 API** 评估。读写由形态四类覆盖；第三方用 **API是否涉及第三方联动**。

| 维 | 信号 | 落盘 |
|:--:|------|------|
| ① | 页数（仅AD） | **独立**：`ui_manifest` / FEATURE↔PAGE 中终端 = AD |
| ② | **API 总数** | 纳入计分的 `API-*` 去重个数（全量 active / 迭代台账涉及） |
| ③ | **API 服务对象** | **单 API**（∈②）：`前端视图` / `资源管理` / `前端视图和资源管理`（禁终端推导） |
| ④ | **API 数据操作形态** | **单 API**（∈②）四类：单对象查询 / 多对象关联查询 / 简单写入 / 多对象写入 |
| ⑤ | **API是否涉及第三方联动** | **单 API**（∈②）：`是` / `否`（是 → 须挂 `EXT-*`） |
| ⑥ | EXT 对接 | **独立**：`EXT-*` + **对接复杂度** |
| ⑦ | FEATURE 分支数与验收标准(AC)条数 | **独立**：FEATURE：**分支数** + AC 表行数 |
...
| 侧 | 要点 |
|----|------|
| FE | ①非AD页基线；**②～⑦按单页**：API数量档位 + 请求编排 + 交互体验 + 视觉细节 + 功能数量档位 + 流程角色 |
| BE | **①⑥⑦独立** + **②API总数×1** + **③④⑤按②中单API**：服务对象 + 数据操作形态(1/2) + API是否涉及第三方联动(否0/是1) + EXT对接复杂度 + FEATURE/AC |
...
- **⑦ check**：**一文档性** | **二功能性** | **三产品就绪度**（含 FE/BE 七维与迭代信号）—— 第三项 **不计点**
...
|----------|--------:|---------:|----------:|----------:|----------:|----------:|----------:|---------:|-----:|
| PAGE-MP-001 | 1 | 1.5（一般） | 0.5（单请求或并行） | 2（标准） | 2（标准） | 0.5（少量） | 1.5（主流程） | 0.6 | 5.4 |
| PAGE-MP-004 | 1 | 0.5（少量） | 0.5（单请求或并行） | 2（标准） | 3（精致） | 0.5（少量） | 1.0（支线） | 1.0 | 8.5 |
...
| 维 | 口径 | 明细摘要 | 迭代系数 | 得分 |
|:--:|------|----------|---------:|-----:|
| ① | AD 页去重数 × 0.5 | 2 页 | 1.0 | 1.0 |
| ⑥ | 各 EXT 对接复杂度之和 | EXT-001 | — | 2.0 |
| ⑦ | 各 FEATURE（分支×0.5 + AC×0.2） | FEATURE-003、FEATURE-010 | — | 3.4 |

### 3.2 单 API（维②③④⑤）

| API编号 | ②计入 | ③服务对象 | ④数据操作形态 | ⑤第三方联动 | 迭代系数 | 小计 |
|---------|------:|----------:|--------------:|------------:|---------:|-----:|
| API-MP-002 | 1 | 1（前端视图） | 2（多对象关联查询） | 0（否） | 0.6 | 2.4 |
| API-MP-010 | 1 | 1.5（前端视图和资源管理） | 2（多对象写入） | 1（是） | 1.0 | 5.5 |
...
**BE_SP** = 1.0 + (2.4+5.5) + 2.0 + (1.1+2.3) = **14.3**

## 4. 迭代调整说明

- PAGE-MP-001、API-MP-002、FEATURE-003：变更形态=`修改`(0.6) × 存量=`无`(1.0) → 系数 **0.6**
- PAGE-MP-004、API-MP-010、FEATURE-010、EXT-001：变更形态=`新建`(1.0) × 存量=`无`(1.0) → 系数 **1.0**
- 摘自 `eval_signals.md`：新建 4 / 修改 3；存量均为「无」
