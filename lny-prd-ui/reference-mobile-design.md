...
列表区：①封面叠字；②双列；③横卡多行（左图/图标/头像/无 + 文本靠左 + 价格/距离右上 + 可选小图）；④横卡单行（每行独立有缝）。**非分组**、数据可无限（分页叠在一起无页边界）；卡统一轻阴影。功能区：**成组**、每组入口有限；金刚宫格 / 金刚双卡（无阴影）/ 通栏 / 一行两个。**设置项**＝当页当行直接操作；**功能入口**＝只跳转或开半屏/弹窗（同壳不同行为）。详见 prototype `reference-kit`。
...
| 类别 | 规格用语 | ⑥ 类名（摘要） |
|------|----------|----------------|
| 基础 | 按钮主/次/线框/浅底/文字/纯链接、输入、列表区 | `md-btn` + `--contained`/`--outlined`/`--soft`/`--text`/`--link`；`md-field`；`md-card--cover` / `--tile` / `--row`；列表单行 `md-stack`>`md-set-row`（独立有缝）。禁止裸 `<button>` 带浏览器皮肤 |
