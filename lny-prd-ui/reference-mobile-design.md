...
列表区：①封面叠字；②双列；③横卡多行（左图/图标/头像/无 + 文本靠左 + 价格/距离右上 + 可选小图；字段多或值长用此；字段少因值长再加大字号、高度随内容）；④横卡单行（仅字段≤3 且值都短；每行独立有缝）。**非分组**、数据可无限（分页叠在一起无页边界）；卡统一轻阴影。功能区：**成组**、每组入口有限；金刚宫格 / 金刚双卡（无阴影）/ 通栏 / 一行两个。**设置项**＝当页当行直接操作；**功能入口**＝只跳转或开半屏/弹窗（同壳不同行为）。详见 prototype `reference-kit`。
...
| 类别 | 规格用语 | ⑥ 类名（摘要） |
|------|----------|----------------|
| 基础 | 按钮主/次/线框/浅底/文字/纯链接、输入、列表区 | `md-btn` + `--contained`/`--outlined`/`--soft`/`--text`/`--link`（纯文字字色须区别紧邻正文）；`md-field`；`md-card--cover` / `--tile` / `--row`；列表单行 `md-stack`>`md-set-row`（独立有缝）。禁止裸 `<button>` 带浏览器皮肤 |
...
| 功能区 | 金刚宫格、金刚双卡、通栏、一行两个 | `md-king` / `md-king--pair`（无阴影）；通栏 `md-set-group`>`md-set-row`（连成一片、轻阴影、组内淡色内缩分割线）；一行两个 `md-set-pair`（分组标题可无、轻阴影）；成组、每组入口有限 |
